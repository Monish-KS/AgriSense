...
const csvFilePath = path.join(__dirname, '../India_data/NoofDealersDistrictWise_upto2013_14.csv');
const jsonFilePath = path.join(__dirname, '../src/data/dealers_data.json');
const results = [];
  ...
  .on('data', (data) => {
    ...
    for (const key in data) {
      if (Object.hasOwnProperty.call(data, key)) {
        ...
        cleanedData[cleanedKey] = isNaN(value) || value === '' ? value : value; // Keep numbers as strings if they might be large or have leading zeros
      }
    }
